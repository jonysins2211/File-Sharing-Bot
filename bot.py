...
# 🧩 Fix: Ignore slash commands globally in text filters
filters.not_command = filters.create(
    lambda _, __, m: not (m.text and m.text.startswith("/"))
)

# 🧩 Compatibility fix for large channel IDs
    ...
    async def start(self):
        ...
        if FORCE_SUB_CHANNEL2:
            try:
                ...
                self.LOGGER(__name__).warning(
                    f"Please Double Check The FORCE_SUB_CHANNEL2 Value And Make Sure Bot Is Admin In Channel With Invite Users Via Link Permission. Current Force Sub Channel Value: {FORCE_SUB_CHANNEL2}"
                )
                self.LOGGER(__name__).info(
                    "\nBot Stopped. https://t.me/MadflixBots_Support For Support"
                )
